...
class OnlyElementsListComponent extends \CBitrixComponent
{
    ...
    protected function checkParamsBoolVal($sValue, $sDefault = 'Y')
    {
        $sValue = trim($sValue);
    ...
    public function executeComponent()
    {
        ...
        if ($this->StartResultCache()) {
            ...
            $this->getIblockPicture();
            if ($this->arParams['ITEMS_BY_SECTION'] == 'Y')
    ...
    public function getIblockPicture(){
        if(!empty($this->arResult['PICTURE'])){
            $this->arResult['PICTURE'] = \CFile::GetFileArray($this->arResult['PICTURE']);
            $this->arResult['PICTURE']['ALT'] = $this->arResult['NAME'];
        }
    }
    ...
    public function getSections()
    {
        ...
        foreach ($this->arResult['ITEMS'] as $arItem) {
            if (!empty($arItem['IBLOCK_SECTION_ID']) && !in_array($arItem['IBLOCK_SECTION_ID'],$arSectionsID))
        ...
        while ($arSection = $rsSection->GetNext(true, false)) {
            ...
            $arSections[$iSectionID] = $arSection;
            $arSections[$iSectionID] ['ITEMS'] = array_filter(
                $this->arResult['ITEMS'],
                function ($element) use ($iSectionID) {
                    return $element['IBLOCK_SECTION_ID']==$iSectionID;
                }
            );
        }
        return $arSections;
     ...
     * @todo доработать для разделовNavStart
     *
     * @param $arItem
     */
